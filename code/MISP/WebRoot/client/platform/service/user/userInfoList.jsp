		               ...
		               <c:forEach var="user" items="${userList}"> 						 
						  <tr>
						    <td style="text-align:center">
								<input type="checkbox" name="selectedIDList"  value="${user.userID}"/>
							</td>
		                    ...
		                    <td style="text-align:center" >
		                      <c:if test="${'userGroupInfo' == pageButtonControl}">
		                         <a id="${user.userID}" class="tip-bottom" href="javascript:void(0);" title="删除" onclick="
								warnModal(
									'删除用户',
									'您确定要删除用户 ${user.userID}的吗？',
									'groupManage!deleteMember.action?selectedID=${user.userID}&operateType=delete'
									)
								"><i class="icon-remove"></i></a>
		                      </c:if>
		                      <c:if test="${'userManage' == pageButtonControl}">
